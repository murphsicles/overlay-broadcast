...
### Added
- **Live node-submission client** (`node` package): a blocking Teranode JSON-RPC client —
...
- **Served HTTP api** (`server` package + `overlay-broadcast-server` binary): a minimal
...
### Added
- Adversarial threat-model suite (`sec` package, Section 16): one executable test per
