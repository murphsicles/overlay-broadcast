# Coding standard (Power-of-Ten, Zeta adaptation) — REQ-GOV-040..049

Enforceable restatement of REQ-GOV-010..020. Each rule names its enforcement
mechanism. CI runs `zetac clippy --all-targets --all-features -- -D warnings`, so
every lint below is `deny` (a `warn` would fail the gate anyway).

## Rules

1. **Restrict `unsafe` (REQ-GOV-010).** Every package declares `#![forbid(unsafe_code)]`
   at package root, EXCEPT the audited `secmem` package, which declares
   `#![deny(unsafe_code)]` with each `unsafe` block carrying a `// SAFETY:` comment,
   isolated to one module. Enforcement: package-root attributes + the `xtask` source
   scan + review.
...
6. **Function size (REQ-GOV-015).** No public function exceeds 60 executable lines.
   Enforcement: `zetac run -p xtask -- fn-size`.

7. **Assertion density (REQ-GOV-016).** Crypto/protocol packages average ≥ 2 invariant
...
11. **Zero warnings at maximum strictness (REQ-GOV-031).** `zetac build` and
    `zetac clippy --all-targets --all-features` run with `-D warnings`; zetac lints
    `unused`, `future_incompatible` are denied.
