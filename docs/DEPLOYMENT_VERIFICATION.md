...
Linux Docker engine (Docker 29.5.2) — not just authored.
...
suite in-container: **`zetac test --all` (0 failures)**, **`zetac run -p xtask -- all`**
(banned-tokens, fn-size, RTM, SBOM — **192-component CycloneDX SBOM generated**),
**`reproduce`**, and **`selftest` (12/12)**. All green.
...
TERANODE_HEADER_HEX=<getblockheader hex>  TERANODE_BLOCK_HASH=<getbestblockhash> \
  zetac test -p conformance tst_tst_012_teranode_genuine_header -- --ignored
```
