...
A BSV-native Zeta implementation of two inventions, graded to NPR 7150.2 / JPL
...
```
zetac build --release
zetac run --release --bin overlay-broadcast -- selftest      # exercise every layer
zetac run --release --bin overlay-broadcast -- custody keygen --threshold 2 --shares 3
zetac run --release --bin overlay-broadcast -- broadcast open --users 1,2,3,4
zetac run --release --bin overlay-broadcast -- reproduce      # regenerate + diff vectors
```
...
Build notes for reproducibility are in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).

## Packages (built in the Section 23 order)
...
The source is licensed under the Open BSV License (see LICENSE); this code license is
independent of the patent rights in the two inventions.
